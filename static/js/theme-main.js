...
$(document).ready(function() {
	...
	$('#ajaxFormMuralSubmit').on('click',function(){
		var Form = $('#ajaxFormMural');
		var hasErrors = Form.validator('validate').has('.has-error').length;
		if (hasErrors){
			...
			$.ajax({
				...
				success: function(msg){
					console.log(msg);
					$('#sendResult').html('<img src="http://marttinfisher.com/themes/bodas/img/form-icon-ok.png"/><br/><span class="title success">Obrigado!</span><br/>Seu recado passará por aprovação dos noivos.<br /><br/><br /><button class="btn btn-default BtnCloseResult" type="button">Fechar</button>')
				},
				error: function(){
					$('#sendResult').html('<img src="http://marttinfisher.com/themes/bodas/img/form-icon-error.png"/><br/><span class="title error">Desculpe!</span><br/>Seus dados não foram enviados. Por favor tente novamente.<br /><strong>Erro: #CL001</strong><br /><br /><button class="btn btn-default BtnCloseResult" type="button">Fechar</button>');
				}
